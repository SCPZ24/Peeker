...
- 展开和收起期间，可见黑色表面必须始终以屏幕上沿和水平中心为固定锚点，宽度、高度与软角连续变化；不得先出现脱离顶部的小框，再在最终帧突然贴顶。
...
- 收敛态基础高度为 `32pt`；带刘海屏取 `max(32pt, safeAreaInsets.top)`，不得使用机型硬编码。
- 带刘海屏的收敛宽度必须容纳左侧摘要、物理刘海和右侧摘要；物理刘海宽度根据屏幕宽度减去左右顶部辅助区域宽度并增加 `4pt` 遮盖余量计算。左右两侧以较宽一翼为基准，各压缩 `64pt` 后对称预留，使物理刘海占位与屏幕中心严格重合；过长内容只能在本翼内截断，不得侵入摄像头区。无刘海屏不得预留中央空洞。
- 展开态安全内容边距为顶部 `16pt`、左右 `28pt`、底部 `28pt`；Timer 展开表面为 `800×328.571pt`，Pusher 为 `960×371.429pt`（高度均为上一版的 `5/7`），小屏按屏幕可用范围约束。
...
- 收敛态左侧显示最近 Timer 任务的“颜色点 + 名称”，右侧显示剩余时长；不显示进度条。达标后右侧显示绿色对勾。
...
- 各列独立滚动。
- 任务状态只能通过拖拽改变。
- 支持跨列迁移和同列手动排序；落点同时决定状态与排序位置。
- 放下时展示层先乐观更新任务位置和摘要，再异步执行一次原子持久化；写入成功不得产生第二次布局跳动。
- 同栏下标按移动前可见列表的插入槽位解释，列尾槽位等于移动前任务数。
- 仅接受当前业务日内由 Peeker 本地 move-only 拖拽会话产生的随机 nonce 信封；外部文本和其他应用数据不得被确认成 move。
- 有效拖拽进入列时必须显示目标列和插入位置，鼠标释放后系统预览落向该目标 frame，同时展示层同步完成乐观落位。
- 移动、任务 CRUD 和跨日恢复共用 Store 级串行边界；若移动期间到达业务日边界，必须在移动成功或回滚后完成恢复，恢复结束前不开放下一次拖拽。
- 新任务默认进入 Planned 末尾。
- 取消或无效拖拽不得执行 CRUD；写入失败时必须恢复本次移动前的原位置，不能产生部分更新或丢失任务。
...
- 收敛态左侧以橙色 Planned 图标和绿色 Done 图标显示数量；右侧不显示 Processing 总数，只显示 Processing 内紧急（红）、推进（蓝）、规划（黄）的三个状态数量。
- 右侧三个状态数量之和必须等于 Processing 总数；Planned 和 Done 任务的急迫度不计入该分解。
- 拖拽、新增、编辑或删除后立即刷新计数。
- 日历 Popover 支持切换月份。
