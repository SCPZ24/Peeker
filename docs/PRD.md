...
- 带刘海屏的收敛宽度必须容纳左侧摘要、物理刘海和右侧摘要；物理刘海宽度根据屏幕宽度减去左右顶部辅助区域宽度并增加 `4pt` 遮盖余量计算。左右两侧按较宽的一翼对称预留，使物理刘海占位与屏幕中心严格重合；无刘海屏不得预留中央空洞。
...
- 收敛态左侧以橙色 Planned 图标和绿色 Done 图标显示数量；右侧显示蓝色 Processing 总数，以及 Processing 内紧急（红）、推进（蓝）、规划（粉红）的数量分解。
- Processing 的三个急迫度数量之和必须等于 Processing 总数；Planned 和 Done 任务的急迫度不计入该分解。
- 拖拽、新增、编辑或删除后立即刷新计数。
- 日历 Popover 支持切换月份。
