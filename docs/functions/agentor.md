...
等待回答不会提高 Agentor 的宿主优先级，也不会抢占当前胜出的 Timer。

### 6.3 Compact 内容与动画

动画只使用 SwiftUI 图形和 SF Symbols，不使用第三方角色、官方品牌素材或 `clawd-on-desk` 资源。

| 执行数量 | 表现 |
|---|---|
| 1 | 单个 Agent 中性 glyph；外围显示 `1.8s` 循环的低亮扫描弧；尾部显示状态短文案 |
| 2 | 两个 glyph 以 `1.6s` 反相呼吸；中央显示数量 `2` |
| ≥3 | 中央数量徽标；三点以 `2.4s` 环绕；文本显示真实数量 |
...
Agentor 展开表面最大为 `960 × 600 pt`。小屏继续使用宿主现有表面裁剪规则；会话和问题内容在卡内滚动。
