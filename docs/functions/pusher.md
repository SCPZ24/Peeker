...
- 支持同列拖拽；落点决定新的列内顺序。
- 跨列拖拽也必须记录目标列中的具体顺序。
- 落点使用移动前目标列的可见插入槽位：列首为 `0`，列尾为当前任务数；同栏移动由领域层在移除源任务后自动修正下标。
- 状态和排序位置在一次原子操作中持久化。
- 放下时先在内存中乐观更新三列和收敛态计数，再异步执行一次完整顺序写入；成功后不做第二次视觉重排。
- 无效落点和取消拖拽不修改数据；持久化失败时，界面以动画恢复到本次操作前的原列和原位置并显示错误。
- 拖拽源使用带随机会话 nonce 的内建 String 信封，AppKit 列容器只接受本应用发起的单项目 `.move`；nonce、业务日和 task ID 任一不匹配均拒绝，不把外部文本当成任务移动。
- 有效任务进入列时显示蓝色列框和插入线；AppKit 将拖拽预览的目标 frame 锚到该插入槽位，乐观 board 在 drop 返回前同步更新。
- Store 在移动写入及其后可能触发的跨日恢复全部结束前持有同一个 mutation lock；这期间不接受第二次移动或其他任务 CRUD，避免失败回滚和旧日恢复覆盖更新后的 board。
- 拖拽期间岛保持展开。
