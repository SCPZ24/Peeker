...
以上颜色用于展开态任务卡；收敛态 Processing 构成中的“规划”计数为提高小尺寸辨识度，单独使用粉红色，不改变任务自身的业务颜色。
...
```text
左侧：Planned 数 | Done 数
右侧：Processing 数 | 紧急数 | 推进数 | 规划数
```

- 统计范围是当前业务日的活跃任务。
- Planned 使用橙色图标，Done 使用绿色图标，Processing 使用蓝色图标。
- 右侧三个急迫度数量只统计 Processing 状态中的任务，分别使用红、蓝、粉红标识；三者之和必须等于 Processing 总数。
...
- [ ] 同列拖拽正确保存新顺序，重启后保持。
- [ ] 无效拖拽和写入失败恢复原列与原位置。
- [ ] 收敛态左侧实时显示 Planned、Done 数量，右侧显示 Processing 总数及其中紧急、推进、规划数量，且急迫度之和等于 Processing。
